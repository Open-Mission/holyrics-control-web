...
import { useImagePresentationStore } from '@/hooks/use-image-presentation-store'
import { getMediaBaseName } from '@/lib/media'
import { cn } from '@/lib/utils'

export function ImagePresentationPanel() {
  const isMobile = useIsMobile()
  const { panelOpen, closePanel } = useImagePresentationStore()

  const content = <ImagePresentationContent />
    ...
    return (
      ...
      >
        <DrawerContent className="h-dvh flex flex-col rounded-none">
          <DrawerTitle className="sr-only">Controle de apresentação de imagens</DrawerTitle>
          {content}
        </DrawerContent>
      </Drawer>
    )
  ...
  return (
    ...
    >
      ...
      >
        <SheetTitle className="sr-only">Controle de apresentação de imagens</SheetTitle>
        {content}
      </SheetContent>
    </Sheet>
  )
...
function ImagePresentationContent() {
  const {
    items,
    selectedIndex,
    presentedIndex,
  ...
  const title = useMemo(() => {
    ...
    return 'Apresentação de imagens'
  ...
  const handlePresentSelected = async () => {
    try {
      if (isPresenting && selectedIndex !== null) {
        await goToImage(selectedIndex)
      } else {
        await startSelectedPresentation()
      }
      toast.success('Apresentação enviada ao Holyrics.')
    } catch {
      toast.error('Erro ao apresentar a imagem selecionada.')
    }
  }
      ...
      <div className="border-b px-4 py-3 shrink-0">
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex items-center gap-2 rounded-xl border bg-muted/40 px-3 py-2 text-sm">
            ...
            <span>
              Selecionada {selectedIndex === null ? 0 : selectedIndex + 1} / {items.length}
            </span>
          </div>
          {isPresenting ? (
          ...
          {(isStarting || isNavigating) ? (
            <div className="inline-flex items-center gap-2 rounded-xl border bg-background px-3 py-2 text-sm text-muted-foreground">
              ...
              <span>{isStarting ? 'iniciando apresentação' : 'trocando imagem'}</span>
            </div>
          ) : null}
        </div>
      </div>

      <div className="grid min-h-0 flex-1 gap-0 md:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
        <div className="border-b md:border-b-0 md:border-r p-4">
          <div className="flex h-full flex-col gap-4">
            <div className="relative flex min-h-[18rem] flex-1 items-center justify-center overflow-hidden rounded-2xl border bg-muted/30">
            ...
            <div className="flex flex-col gap-1">
              <h3 className="truncate text-base font-semibold">
                {previewItem?.name ?? 'Nenhuma imagem selecionada'}
              </h3>
              <p className="truncate text-sm text-muted-foreground">
                {previewItem?.path ?? 'Selecione uma imagem na lista para visualizar.'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {isPresenting ? 'Apresentar selecionada' : 'Iniciar apresentação'}
              </Button>
              <Button
                variant="outline"
                onClick={closePanel}
