...
export function GlobalPresentationSync() {
  const { song: currentSong, activeIndex: currentActiveIndex } = usePresentationStore()
  ...
  useEffect(() => {
    const sync = async () => {
      ...
      if (!presentation || presentation.status === 'none' || !presentation.id) {
        if (currentSong) {
          setState({ song: null, slides: [], activeIndex: null, presentationKind: null })
        }
        return
      }
      ...
      if (presentation.type !== 'song' && presentation.type !== 'verse' && presentation.type !== 'lyrics') {
        return
      }
      ...
      try {
        ...
        if (!songDetail || !songDetail.slides || songDetail.slides.length === 0) {
        ...
        console.error('[GlobalPresentationSync] Failed to sync presentation details:', err)
      }
    }
