...
const VIEWS = [
  ...
  { id: "text-aux-control", label: "Text Aux Control", path: "/view/text-aux-control" },
  { id: "text2", label: "Text 2", path: "/view/text2" },
  { id: "text-aux-control2", label: "Text Aux Control 2", path: "/view/text-aux-control2" },
  { id: "text3", label: "Text 3", path: "/view/text3" },
  { id: "text-aux-control3", label: "Text Aux Control 3", path: "/view/text-aux-control3" },
...
export function ProjectionPreview() {
  ...
  const baseUrl = React.useMemo(() => {
    const storedUrl = typeof localStorage !== 'undefined' ? localStorage.getItem('HOLYRICS_URL') : null
    ...
    const serverData = readRecord(
      serverInfoResponse && typeof serverInfoResponse === "object" && "data" in serverInfoResponse
        ? serverInfoResponse.data
        : undefined
    )
    const ip = serverData?.ip_list?.[0] || serverData?.ip || serverData?.address || window.location.hostname
  ...
  return (
    <>
      <SurfaceCard className="flex flex-col rounded-2xl border">
        <CardHeader className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div className="space-y-1">
            ...
            <CardDescription>Visualize as saídas do Holyrics com foco no que está no ar agora.</CardDescription>
          </div>
          <div className="flex flex-col gap-3 sm:items-end">
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-2 rounded-xl border bg-muted/60 px-3 py-1.5">
                <Switch
                  id="dual-view"
                  checked={dualView}
                  ...
                  size="sm"
                />
                <Label htmlFor="dual-view" className="flex cursor-pointer items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            ...
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              <Select value={view1} onValueChange={setView1}>
                <SelectTrigger size="sm" className="w-full rounded-xl sm:w-[190px]">
        ...
        <CardContent className="flex min-h-[300px] flex-1">
          <PreviewGrid
            dualView={dualView}
            view1={view1}
            view2={view2}
      ...
      <Drawer open={isFullscreenOpen} onOpenChange={setIsFullscreenOpen} direction="bottom">
...
function readRecord(value: unknown) {
  ...
  const ipList = Array.isArray(source.ip_list)
    ? source.ip_list.filter((item): item is string => typeof item === "string" && item.trim().length > 0)
    : undefined
...
function PreviewFrame({
  ...
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-2xl border bg-card shadow-inner",
        fullscreen ? "min-h-[320px] sm:min-h-[420px]" : "aspect-video"
      )}
    >
      <iframe
        src={src}
